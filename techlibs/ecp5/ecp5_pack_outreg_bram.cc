...
struct Ecp5PackOutRegBRAMPass : public Pass {
	...
	)
	{
		...
		if (ff_has_en && sigmap(trellis_ff->getPort(ID(CE))) != required_en) {
			log("\tEnable used for this TRELLIS_FF is different then the one used for the first TRELLIS_FF.\n");
			return false;
		}

		if (!ff_has_en && required_en != SigBit(true)) {
			log("\tThis TRELLIS_FF doesn't have an Enable while the first TRELLIS_FF has an Enable\n");
			return false;
		}
	...
	)
	{
		...
		for (auto dout : datout_ports) {
			...
			log("\tDisconnecting TRELLIS_FF %s.\n", log_id(receiver_cell->name));
	...
	)
	{
		...
		for (auto dout : datout_ports) {
			...
			if (!ff_is_mergeable(sigmap, dp16kd, id_clk, id_rst, id_clkmux, first_en, receiver_cell))
				return;
		...
		pack_ff_into_dp16kd(module, receivers, datout_ports, dp16kd, id_oce, first_en, id_regmode);
	}
	...
	void try_pack_outreg_dp16dk(
		...
		if (output_reg_a == false)
			try_pack_port(module, receivers, sigmap, dp16kd, ID(CLKA), ID(RSTA), ID(OCEA), ID(CLKAMUX), ID(REGMODE_A), ID_DOA);
		else
		...
		if (output_reg_b == false)
			try_pack_port(module, receivers, sigmap, dp16kd, ID(CLKB), ID(RSTB), ID(OCEB), ID(CLKBMUX), ID(REGMODE_B), ID_DOB);
		else
			log("\tHas OUTREG already. Skipping...\n");
